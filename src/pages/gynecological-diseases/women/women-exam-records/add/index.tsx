...
export class Panel extends PanelWithChild {
  ...
  handleChangeTab = async (activeKey: string) => {
    ...
    const patientId = get(this.props, 'location.query.patientId');
    let data = {};
    if (id) {
      data = await request.get(`/gynecological-visits/${id}`);
    } else if (patientId) {
      data = {
        gynecologicalPatient: await request.get(`/gynecological-patients/${patientId}`),
      };
    }
    this.setState({
      activeKey,
...
export default connect(({ ids }) => ({
  womenExamRecordsEditingId: get(ids, 'womenExamRecordsEditingId'),
}))(Panel);
